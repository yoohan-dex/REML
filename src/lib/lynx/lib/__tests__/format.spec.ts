...
describe('format', () => {
  it('should can format the string perfectly', () => {
    ...
    const str = '<closing style="background-color: #333;" class="heloo hhh">'
      + '<Hello src="https://www.baidu.com">hello</Hello></closing>';
  ...
  it('should can format the some attribute like if and for', () => {
    ...
    const str =
      '<closing style="background-color: {{primary}}" re:if="{{isRight}}">'
      + '<Hello re:for="{{loop}}">hello</Hello></closing>';
    let tokens = lexer(str, options);
    const nodes = parser(tokens, options);
    const result = format(nodes);
    expect(result).toEqual([
  ...
  it('should take id away', () => {
    ...
    const str =
      '<closing id="abc"><h1>hey</h1><Hello re:for="{{loop}}">hello</Hello></closing>';
    let tokens = lexer(str, options);
    const nodes = parser(tokens, options);
    const result = format(nodes);
    expect(result).toEqual([
